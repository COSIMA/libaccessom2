...
contains
    ...
    procedure, pass(self), public :: update_fields => forcing_update_fields
endtype forcing
...
subroutine forcing_update_fields(self, cur_date, fields)
    ...
    if (fields(i)%timestamp == cur_date) then
        return
    endif
    ...
    do i=1, size(fields)
        ...
        filename = filename_for_year(fields(i)%filename, year)
        ...
        ! Get variable ID
        call ncheck(nf90_inq_varid(ncid, fields(i)%ncname, varid), &
                    'Inquire: '//trim(varname))

        call read_data(ncid, varid, varname, indx, fields(i)%data_array)

        call ncheck(nf90_close(ncid), 'Closing '//trim(filename))

        fields(i)%timestamp = cur_date
    enddo

endsubroutine forcing_update_fields
